...
import type { IIngreso, ICreateIngreso } from "../interfaces/IIngreso";
...
export class IngresosDataSource {
  ...
  async create(ingreso: ICreateIngreso): Promise<IIngreso> {
    const respuesta = await this.httpClient.post<IIngreso>(
      API_ROUTES.INGRESOS.CREATE,
      ingreso
    );
    return respuesta.datos;
  }
